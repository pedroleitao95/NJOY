...
//            UUID.fromString(SampleGattAttributes.HEART_RATE_MEASUREMENT);
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            String intentAction;
//Log.d("NJOY", "BluetoothGattCallback.onConnectionStateChange newState: " + newState);
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                intentAction = ACTION_GATT_CONNECTED;
                mConnectionState = STATE_CONNECTED;
                broadcastUpdate(intentAction);

                // Log.i(TAG, "BluetoothGattCallback.onConnectionStateChange Connected to GATT server.");
                ...
                //Log.i(TAG, "BluetoothGattCallback.onConnectionStateChange disconnected from GATT server.");
                broadcastUpdate(intentAction);
            }
        }
...
//Log.d("NJOY", "BluetoothGattCallback.onServicesDiscovered status: " + status);
            if (status == BluetoothGatt.GATT_SUCCESS) {
                broadcastUpdate(ACTION_GATT_SERVICES_DISCOVERED);
            }
            else {
                Log.w(TAG, "BluetoothGattCallback.onServicesDiscovered received status: " + status);
            }
        }

        @Override
        public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
//Log.d("NJOY", "BluetoothGattCallback.onCharacteristicWrite status: " + status);
            //this block should be synchronized to prevent the function overloading
            synchronized(this) {
                //CharacteristicWrite success
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    //Log.d("NJOY", "BluetoothGattCallbackonCharacteristicWrite success:"+ new String(characteristic.getValue()));
                    if (mCharacteristicRingBuffer.isEmpty()) {
                        ...
                        if(bluetoothGattCharacteristicHelper.mCharacteristicValue.length() > MAX_CHARACTERISTIC_LENGTH)
                        {
                            ...
                            if (mBluetoothGatt.writeCharacteristic(bluetoothGattCharacteristicHelper.mCharacteristic)) {
                                // Log.d("NJOY", "writeCharacteristic init "+new String(bluetoothGattCharacteristicHelper.mCharacteristic.getValue())+ ":success");
                            }
                            else {
                                // Log.d("NJOY", "writeCharacteristic init "+new String(bluetoothGattCharacteristicHelper.mCharacteristic.getValue())+ ":failure");
                            }
                ...
                //WRITE a NEW CHARACTERISTIC
                    ...
                    mCharacteristicRingBuffer.clear();
                    Log.d("NJOY", "onCharacteristicWrite fail:"+ new String(characteristic.getValue()));
                }
            }
        }
        ...
        public void onCharacteristicRead(BluetoothGatt gatt,
                                         ...
                                         int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
//Log.d("NJOY", "BluetoothGattCallback.onCharacteristicRead  "+characteristic.getUuid().toString());
        ...
        public void  onDescriptorWrite(BluetoothGatt gatt,
                                       ...
                                       int status){
//Log.d("NJOY", "BluetoothGattCallback.onDescriptorWrite  "+characteristic.getUuid().toString()+" "+status);
        }
        @Override
        public void onCharacteristicChanged(BluetoothGatt gatt,
                                            BluetoothGattCharacteristic characteristic) {
//Log.d("NJOY", "BluetoothGattCallback.onDescriptorWrite  "+characteristic.getUuid().toString()+" ");
    ...
    private void broadcastUpdate(String action, BluetoothGattCharacteristic characteristic) {
        //Log.d("NJOY", "BluetoothLeService broadcastUpdate");
        Intent intent = new Intent(action);
    ...
    public boolean initialize() {
        ...
        if (mBluetoothManager == null) {
            ...
            if (mBluetoothManager == null) {
                Log.e(TAG, "BluetoothLeService.initialize Unable to initialize BluetoothManager.");
                return false;
            }
        }
        ...
        if (mBluetoothAdapter == null) {
            Log.e(TAG, "BluetoothLeService.initialize Unable to obtain a BluetoothAdapter.");
            return false;
        }
    ...
    public boolean connect(final String address) {
        Log.d("NJOY", "BluetoothLeService.connect address: " + address + ", " + mBluetoothGatt);
        if (mBluetoothAdapter == null || address == null) {
            Log.w(TAG, "BluetoothLeService.connect bluetoothAdapter not initialized or unspecified address.");
            return false;
        }

        final BluetoothDevice device = mBluetoothAdapter.getRemoteDevice(address);
        if (device == null) {
            Log.w(TAG, "BluetoothLeService.connect Device not found. Unable to connect.");
            return false;
        }
    ...
    public void disconnect() {
        Log.d("NJOY", "BluetoothLeService.disconnect");
        if (mBluetoothAdapter == null || mBluetoothGatt == null) {
            Log.w(TAG, "BluetoothLeService.disconnect bluetoothAdapter not initialized");
            return;
        }
     ...
     */
    public void close() {
        Log.d("NJOY", "BluetoothLeService.close");
    ...
    public void readCharacteristic(BluetoothGattCharacteristic characteristic) {
        if (mBluetoothAdapter == null || mBluetoothGatt == null) {
            Log.w(TAG, "BluetoothLeService.readCharacteristic bluetoothAdapter not initialized");
            return;
        }
    ...
    public void writeCharacteristic(BluetoothGattCharacteristic characteristic) {
        if (mBluetoothAdapter == null || mBluetoothGatt == null) {
            Log.w(TAG, "BluetoothLeService.writeCharacteristic bluetoothAdapter not initialized");
            return;
        }
        ...
        //Log.d("NJOY", "allwriteCharacteristicString:"+writeCharacteristicString);
        ...
        //Log.d("NJOY", "mCharacteristicRingBufferlength:"+mCharacteristicRingBuffer.size());
    ...
    public void setCharacteristicNotification(BluetoothGattCharacteristic characteristic, boolean enabled) {
        if (mBluetoothAdapter == null || mBluetoothGatt == null) {
            Log.w(TAG, "BluetoothLeService.writeCharacteristicBluetoothAdapter not initialized");
            return;
        }
